...
def calibrate(nchains, model, init_bounds, num_steps, discard, percentile, chain_name):
    ...
    if not np.allclose(new_bounds, init_bounds):
        ...
        # save chains
        samples = sampler.get_chain()
        np.save('MCF7_control_chains_' + str(chain_name) + '.npy', samples)
        # save flat chains
        flat_samples = flatten(sampler, discard)
        np.save('MCF7_control_fchains_' + str(chain_name) + '.npy', flat_samples)
...
FilenamesList = sorted(glob.glob('./cell_GH1818_*.txt'))
data_list = []
for file in FilenamesList:
    u = np.loadtxt(file)
    data_list.append(u)
treatment = np.loadtxt('./treatment_GH1818.txt')
...
# Crop the data above 800 hours
for j in range(len(means)):
    pos = np.argmax(means[j][0]>800)
    means[j] = means[j][:,0:pos]
# Crop the data above 30000 cells
for j in range(len(means)):
    pos = np.argmax(means[j][1]>30000)
    if pos>0:
        means[j] = means[j][:,0:pos]
...
# plt.ylim(0, 5000)
plt.title("MCF7 Control Calibration")
