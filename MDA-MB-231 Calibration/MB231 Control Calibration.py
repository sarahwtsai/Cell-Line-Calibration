...
def calibrate(nchains, model, init_bounds, num_steps, discard, percentile, chain_name):
    ...
    if not np.allclose(new_bounds, init_bounds):
        ...
        # save chains
        samples = sampler.get_chain()
        np.save('MB231_control_chains_' + str(chain_name) + '.npy', samples)
        # save flat chains
        flat_samples = flatten(sampler, discard)
        np.save('MB231_control_fchains_' + str(chain_name) + '.npy', flat_samples)
...
FilenamesList = sorted(glob.glob('./cell_MB231_*.txt'))
data_list = []
...
# Crop the data above 25000 cells
for j in range(len(means)):
    pos = np.argmax(means[j][1]>25000)
    if pos>0:
        means[j] = means[j][:,0:pos]
...
# plt.ylim(0, 5000)
plt.title("MDA-MB-231 Control Calibration")
